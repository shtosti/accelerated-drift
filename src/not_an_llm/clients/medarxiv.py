...
class MedarxivClient:
    base_url: str = "https://api.medrxiv.org/details/medrxiv"
    ...
    def _get_feed(self, *, start_date: str, end_date: str, cursor: int) -> dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                if isinstance(payload, dict):
                    return payload
                raise ValueError("Unexpected medRxiv response payload type")
                ...
                logger.warning(
                    "Retryable HTTP %s from medRxiv (attempt %s/%s). sleeping %.2fs",
                    status_code,
                    attempt + 1,
                    self.max_retries + 1,
                    sleep_seconds,
                ...
                logger.warning(
                    "Network error from medRxiv (attempt %s/%s). sleeping %.2fs",
                    attempt + 1,
                    self.max_retries + 1,
                    sleep_seconds,
                )
        ...
        raise RuntimeError("medRxiv request retry loop exhausted unexpectedly")
...
def _normalize_record(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "paperId": doi or f"medrxiv:{title}|{published_at.date().isoformat()}",
        "title": title,
        "abstract": abstract,
