...
### Shared accessible visual style

All analysis and comparison plotting modules import their visual encodings from
`src/not_an_llm/analysis/visual_style.py`. This is the single control point for
project colors, diverging colormaps, dataset colors, corpus line styles, marker
shapes, and hatch patterns.

The project retains orchid and green as its visual identity. The shared green is
blue-leaning teal so that it separates more reliably from orchid under common
red-green color-vision deficiencies. Color is never intended to be the only
category encoding:

- corpus time series combine color with solid, dashed, dotted, and dash-dot lines;
- topic and dependency series combine color with marker shape and direct labels;
- grouped bars combine dataset color with hatch pattern;
- negative effect bars use orchid plus diagonal hatching, while positive bars use
  teal plus an unhatched fill;
- diverging heatmaps use orchid-white-teal, are centered at zero, and retain
  printed numeric cell values.

When changing the project palette, edit `visual_style.py` rather than introducing
local hexadecimal colors in plotting modules. Figures should remain interpretable
in greyscale through their labels, shapes, hatches, or line styles.
